...
{\color{blue}These estimated values for the magnetic field strength should be considered the maximum attainable field strengths given the particle content of the primordial QGP. The actual magnetization of the quarks and leptons is likely weaker due to the high temperatures of the cosmic plasma which tends to disrupt the necessary alignment required for magnetization. \rf{Figure_1} also reveals that the heaviest considered particles such as bottom or tau have densities during this era requiring large polarization fractions to account for the primordial fields unlike the lighter species. Heavier species behaving semi- or non-relativistically, however, may proportionally be easier to magnetize due to the minimization of free energy of the system.

We can estimate the polarization factor based on our scaling of the primordial fields in \req{eq:PMF}. We understand the degree of polarization to be an aberration of the spin degrees of freedom available to the species. E.g.
\begin{equation}
...
The power and utility of the partition function in statistical systems is found by examining the Fermi integral in different limits and expansions. We state the Fermi-Dirac distribution given by
\begin{align}
\label{eq:FermiDirac}
F\left(E - \sigma\eta\right) = \frac{1}{e^{(E - \sigma\eta)/T} + 1}\,.
\end{align}
The parameter \(\sigma \in {\pm1}\) describes both matter and antimatter states satisfying \req{eq:equilibirum_conditions}. We can evaluate \req{eq:parts} by utilizing Euler-Maclaurin integration over the Landau levels \(n\) (see details in Ref.~\cite{Steinmetz:2023nsc}) and write the partition function in spherical coordinates \(d\mathbf{p}^{3}=4\pi p^{2}dp\) {\color{blue}given by 
\begin{equation}
