...
\end{tabular}
\end{table}
In Table.~\ref{tab:particle_properties}, we list the properties of select particles relevant to the QGP era. The magneton \(\mu\) is the magnetic moment of the particle in units of the Bohr magneton \(\mu_{B}\). The number of degrees of freedom \(\mathrm{dof.}\) is the number of spin and color states available to the particle. The magneton is a measure of the strength of the magnetic dipole moment of a particle where \(g\)-factor has been set to \(g=2\).
