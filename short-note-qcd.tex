...
%%%%%%%%%%%%%%%%%%%%%%%%
{\color{blue}We address the mystery of cosmological sized \((\sim1\,{\rm Mpc})\) magnetic fields in the Universe. The origin of these fields is not fully understood, but there is little doubt in their existence from observations. Notably, it is hard to destroy magnetic fields once generated, therefore one can extrapolate present day observation of primordial magnetic fields (PMF) to the primordial epoch.} Recently we have {\color{blue}proposed} the possibility that cosmic magnetism originates in spin polarization of electron-positron pairs~\cite{Steinmetz:2023nsc,Steinmetz:2023ucp} near to the Big-Bang Nucleosynthesis epoch~\cite{Grayson:2023flr,Grayson:2024uwg}. We now approach the possible role of light quark-antiquark pairs in the QGP (quark-gluon plasma) phase near to hadronization, pushing the possible source of spin magnetization further back in time to before matter hadronization.

\begin{figure}%[b]
\centerline{
\includegraphics[width=0.90\columnwidth]{Figure_1_v3.pdf}}
\caption{\label{Figure_1}Temperature dependence of several key magnetic field contributions in the early universe during the QGP epoch. {\color{blue}The PMF range (grey band) was obtained from \req{eq:PMF}. The maximum possible spin magnetization for quarks are plotted with the following curves: light \(u+d\) quarks (orange), strange (bright red), charm (dark red), and bottom (brown). Lepton curves are given by: electrons (dark blue), muons (blue), and tau (light blue). The maximum magnetic field strengths from spin polarization \(\mathcal{B}\) for charged leptons and quarks was determined by \req{eq:UpQuarkMagnetization} and \req{eq:ElectronMagnetization} respectively.}}
\end{figure}

{\color{blue} What makes the QGP epoch particularly attractive for study is the high matter-antimatter density involved. This gives the QGP epoch an advantage over the electron-positron epoch in that the relative amount of polarization required to generate PMFs is much lower.} This is shown in Fig.~\ref{Figure_1} over the temperature range 5 to 500~MeV. {\color{blue} The maximum spin magnetization is shown with differing colored curves for each species. The gray-shaded region represents the allowed PMF range, obtained by scaling today’s intergalactic magnetic field (bounded between \(10^{-12}~{\rm T} > \mathcal{B}_{0} > 10^{-20}~{\rm T}\))~\cite{Planck:2015zrl,Jedamzik:2018itu} via
\begin{equation}
\label{eq:PMF}
...
\end{align}
where \(M=\mathcal{M}/V\) is the magnetic moment density, \(\mathcal{M}\) is the magnetization, and \(\mu_{0}\) is the vacuum permeability (not to be confused with magnetic moment). {\color{blue}Therefore at \(k_{B}T=300~\mathrm{MeV}\), which is where the perturbative QCD regime sets in, and using \req{eq:NumberDensity} and \req{eq:TotalMagnetization}, we obtain an upper bound for the up-quark contribution to magnetic field strength
\begin{align}
\label{eq:UpQuarkMagnetization}
&\mathcal{B}_{u\bar{u}}(T) < \mu_{0}\mu_{u}[n_{u}(T)+n_{\bar{u}}(T)]=\mu_{0}\mu_{u}\frac{\mathfrak{g}_{u}}{\pi^{2}}\frac{3\zeta(3)}{2}\left(\frac{k_{B}T}{\hbar c}\right)^{3}\,,\\
\label{eq:estimate}
&\mathcal{B}_{u\bar{u}}(300\MeV) < 7.0\times10^{15}~\mathrm{T} \,.
\end{align}
